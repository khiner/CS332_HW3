...
class Fundef {
    ...
    public VarEnv extend(Id id, Type type, VarEnv env) {
        ...
        return new VarEnv(id, type, offset, false, env);
    }
    ...
    void check(Context ctxt)
      throws Failure {
        if (body!=null) {
            body.check(this, ctxt, null, Formals.buildEnv(formals));
    ...
    void compile(Assembly a) {
        if (body!=null) {
            ...
            body.compile(a, a.emitPrologue(localBytes));
            if (!returns) {
                a.emitEpilogue();
            }
        }
